...
var (
	...
	serviceTag = prometheus.NewDesc(
		...
		"Tags of a service.",
		[]string{"service_id", "node", "tag","addr"}, nil,
	)
	serviceNodesHealthy = prometheus.NewDesc(
		...
		[]string{"service_id", "node", "service_name","addr"}, nil,
	)
	nodeChecks = prometheus.NewDesc(
		...
		[]string{"check", "node", "status","addr"}, nil,
	)
...
func (e *Exporter) collectNodesMetric(ch chan<- prometheus.Metric) bool {
	nodesInfo, _, err := e.client.Catalog().Nodes(&queryOptions)
	weight:=1.
	if err != nil {
	...
	ch <- prometheus.MustNewConstMetric(
		nodeCount, prometheus.GaugeValue, float64(len(nodesInfo)),
	)
	for _, info:= range nodesInfo {
		ch <- prometheus.MustNewConstMetric(
			nodes,prometheus.GaugeValue,weight,info.Node,info.Address,)
	}
...
func (e *Exporter) collectHealthStateMetric(ch chan<- prometheus.Metric) bool {
	...
	for _, hc := range checks {
		...
		if hc.ServiceID == "" {
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, passing, hc.CheckID, hc.Node, consul_api.HealthPassing,"das",
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, warning, hc.CheckID, hc.Node, consul_api.HealthWarning,"dasa",
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, critical, hc.CheckID, hc.Node, consul_api.HealthCritical,"sdasd",
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, maintenance, hc.CheckID, hc.Node, consul_api.HealthMaint,"sdasfa",
			)
		} else {
			ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectOneHealthSummary(ch chan<- prometheus.Metric, serviceName string) bool {
	...
	for _, entry := range service {
		...
		ch <- prometheus.MustNewConstMetric(
			serviceNodesHealthy, prometheus.GaugeValue, passing, entry.Service.ID, entry.Node.Node, entry.Service.Service,entry.Node.Address,
		)
		...
		for _, tag := range entry.Service.Tags {
			...
			ch <- prometheus.MustNewConstMetric(serviceTag, prometheus.GaugeValue, 1, entry.Service.ID, entry.Node.Node, tag,entry.Node.Address,)
			tags[tag] = struct{}{}
		}
	}
	return true
